...
Validator.prototype.check = function(test, tag, params, cb) {
   ...
   if (this.errors.length) {
      if (this.res) {
         if (this.errors[0].tab === Validator.Tags.noLogin)
...
Validator.prototype.checkPrsOK = function(prsId, cb) {
   ...
   return this.check(this.session, Validator.Tags.noLogin, null, cb) &&
    this.check(this.session.isAdmin() || this.session.id == prsId,
    Validator.Tags.noPermission, null, cb);
};
...
Validator.prototype.hasOnlyFields = function(obj, fieldList) {
    ...
    Object.keys(obj).forEach(function(prop){
        self.chain(fieldList.indexOf(prop) >= 0, Validator.Tags.forbiddenField,
         [prop]);
    });
